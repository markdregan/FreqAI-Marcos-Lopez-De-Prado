...
class LitmusBBTrendStrategy(LitmusSimpleStrategy):
    """
    to run this:
      freqtrade trade --strategy LitmusBBTrendStrategy
    ...
    def feature_engineering_standard(self, dataframe, **kwargs) -> pd.DataFrame:
        ...
        *Only functional with FreqAI enabled strategies*
        ...
        dataframe["rsi-ma"] = dataframe["rsi"].rolling(window=7).mean()

        # Primary Entry Logic (shift entry forward +1 step)
        dataframe["primary_enter_long"] = (
            (dataframe["close"] > dataframe["mm_bb_middleband"]) &
            (qtpylib.crossed_above(dataframe["rsi-ma"], dataframe["rsi-ma"].shift(1)))
        )
        ...
        dataframe["primary_enter_short"] = (
            (dataframe["close"] < dataframe["mm_bb_middleband"]) &
            (qtpylib.crossed_below(dataframe["rsi-ma"], dataframe["rsi-ma"].shift(1)))
        )
        dataframe["primary_enter_short"] = dataframe["primary_enter_short"].shift(1).fillna(
            value=False)

        # Primary Exit Logic
        dataframe["primary_exit_long"] = False
        dataframe["primary_exit_short"] = False
